...
def get_media_message(message_id):
    try:
        ...
        if message and message.get("is_media"):
            # Если сообщение помечено как невалидное
            if message.get("media_invalid"):
                return {
                    "success": False, 
            ...
            if "file_id" in message:
                ...
                try:
                    if fs.exists(message["file_id"]):
                        ...
                        # Помечаем сообщение как невалидное
                        messages_collection.update_one(
                            {"_id": ObjectId(message_id)},
                            {"$set": {"media_invalid": True}}
                        )
                        return {
                            "success": False, 
                            "message": "Файл недоступен",
                    ...
                    logger.warning(f"Файл не найден в GridFS для сообщения {message_id}")
                    return {
                        "success": False, 
                        "message": "Файл недоступен",
                        "file_unavailable": True
            ...
            # Если нет file_id, ищем в старом формате
            ...
            return {
                "success": False, 
                "message": "Данные медиа не найдены",
                "file_unavailable": True
            }
